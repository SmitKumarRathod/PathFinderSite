...
function checkNode(row, col, curr, checker, seen, counter) {
  if(row >= 0 && col >= 0 && row <= 9 && col <= 9) {
    ...
    var cost = Math.min(
      parseInt(curr.getAttribute("cost")) + parseInt(node.getAttribute("weight")),
...
function changeColor(node, counter, cost) {
  setTimeout(() => {
    node.style.backgroundColor = "#8ebfbf";
    if (cost) {
      node.innerHTML = cost;
    }
  }, counter * 100);
...
function start() {
  ...
  setTimeout(() => {
    startNode.style.backgroundColor = "#666";
    startNode.style.color = "#fff";
    startNode.style.font= "large";
    while (endNode.getAttribute("parent") != "null") {
      endNode.style.backgroundColor = "#666";
      endNode.style.color = "#fff";
      endNode.style.font= "large";
      var coor = endNode.getAttribute("parent").split("|");
      var prow = parseInt(coor[0]);
      var pcol = parseInt(coor[1]);
      endNode = document.querySelector(`div[row="${prow}"][col="${pcol}"]`);
